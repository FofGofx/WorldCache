...
def generate(args):
    ...
    device = local_rank
    _init_logging(rank)
    worldcache_mode = os.getenv("WORLDCACHE_MODE")

    if worldcache_mode is not None:
        if worldcache_mode not in {"original", "worldcache"}:
            raise ValueError(
                f"Unsupported WORLDCACHE_MODE={worldcache_mode}. "
                "Use 'original' or 'worldcache'."
            )
        if world_size != 1:
            raise NotImplementedError(
                "LingBot WorldCache adaptation currently supports only single-GPU runs."
            )
        logging.info(f"LingBot WorldCache requested with mode={worldcache_mode}")
