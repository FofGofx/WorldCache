#WorldScore/models/HunyuanWorld-Voyager/voyager/pipelines/pipeline_hunyuan_video.py
...
Please use VaeImageProcessor.postprocess(...) instead"
    ...
    def __call__(
        ...
        **kwargs,
        ...
        #check print
        print(f"[pipe] call() got height={height}, width={width}, video_length={video_length}, vae_ver={vae_ver}")
        ...
        #check print
        print(f"[pipe] call() adjusted video_length for vae_ver={vae_ver}: {video_length}")
...
#check print
        print(f"[pipe] prepared latents shape={latents.shape}") # [B, C, F_lat, H_lat, W_lat]
        if i2v_mode and i2v_condition_type == "latent_concat":
            if img_latents.shape[2] == 1:
        ...
        # Record denoising time using progress bar scope
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i2v_mode and i2v_condition_type == "latent_concat":
                    ...
                    #check print
                    print(f"[pipe] latent_model_input shape={latent_model_input.shape}")
                else:
        ...
        # Write denoising_time.txt if output_dir is provided
        if output_dir is not None:
            ...
            try:
                ...
                logger.warning(f"Failed to write denoising_time.txt to {denoising_time_file}: {e}")
...
#check print
            print(f"[pipe] latents before VAE decode shape={latents.shape}")
            with torch.autocast(
...
#check print
                print(f"[pipe] decoded image shape(before postprocess)={image.shape}") # [B, C, T, H, W]
        else:
            image = latents
        ...
        if i2v_mode and i2v_condition_type == "latent_concat":
            ...
            pass
        #check print
        print(f"[pipe] image shape={image.shape}")
...
#check print
        image = rgb
        print(f"[pipe] final videos tensor shape(out)={image.shape}") # 这里是返回到 predict 的 videos
        # Offload all models
