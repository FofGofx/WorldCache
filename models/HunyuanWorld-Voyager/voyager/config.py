...
def _default_model_base() -> str:
    env_base = os.getenv("MODEL_BASE")
    if env_base:
        return env_base
    model_path = os.getenv("MODEL_PATH")
    if model_path:
        return str(Path(model_path) / "HunyuanWorld-Voyager" / "ckpts")
    worldscore_path = os.getenv("WORLDSCORE_PATH")
    if worldscore_path:
        return str(Path(worldscore_path) / "models" / "HunyuanWorld-Voyager" / "ckpts")
    return str(Path(__file__).resolve().parents[1] / "ckpts")
