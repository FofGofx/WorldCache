...
# =================== Model Path =====================
def _resolve_voyager_root() -> Path:
    env_root = os.getenv("VOYAGER_ROOT")
    if env_root:
        return Path(env_root)
    model_path = os.getenv("MODEL_PATH")
    if model_path:
        return Path(model_path) / "HunyuanWorld-Voyager"
    worldscore_path = os.getenv("WORLDSCORE_PATH")
    if worldscore_path:
        return Path(worldscore_path) / "models" / "HunyuanWorld-Voyager"
    return Path(__file__).resolve().parents[1]


VOYAGER_ROOT = _resolve_voyager_root()
MODEL_BASE = os.getenv("MODEL_BASE", str(VOYAGER_ROOT / "ckpts"))
