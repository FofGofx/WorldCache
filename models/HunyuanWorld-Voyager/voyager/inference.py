#WorldScore/models/voyager/inference.py
...
class HunyuanVideoSampler(Inference):
    def __init__(
        ...
        try:
            from voyager.worldcache_voyager.forwards import (
                apply_worldcache_voyager_pipeline,
                apply_worldcache_voyager_forward
            )
            apply_worldcache_voyager_pipeline(self.pipeline)
            apply_worldcache_voyager_forward(self.pipeline.transformer)
            
            # Set mode from environment variable or args, default to 'worldcache'
            import os
            mode = os.environ.get('WORLDCACHE_MODE', 'worldcache')
            # Check if mode is set in args
            if hasattr(args, 'worldcache_mode') and args.worldcache_mode:
                mode = args.worldcache_mode
            # Set mode on pipeline
            self.pipeline.worldcache_mode = mode
            
            logger.info(f"Worldcache acceleration applied to Voyager (mode: {mode})")
        except Exception as e:
            logger.warning(f"Failed to apply Worldcache acceleration: {e}")
...
#check print
        print(f"[predict] input video_length={video_length}")
        print(f"[predict] target_height(before *2+16)={height}, target_width={width}")
...
#check print
        print(f"[predict] target_height(after)={target_height}, target_width={target_width}")
...
#check print
        print(f"[predict] ref_images_pixel_values shape={ref_images_pixel_values.shape}") # [B(=1), C, F(=1), H, W]
        print(f"[predict] partial_cond pixel shape(before VAE)={partial_cond.shape}") # [1, C, F(=49), H, W]
        print(f"[predict] partial_mask pixel shape(before VAE)={partial_mask.shape}") # [1, C, F(=49), H, W]
        ...
        with torch.autocast(device_type="cuda", dtype=torch.float16, enabled=True):
            ...
            #check print
            print(f"[predict] ref_latents shape={ref_latents.shape}") # -> [1, C, F_lat, H_lat, W_lat]
            print(f"[predict] partial_cond latents shape={partial_cond.shape}")
            print(f"[predict] mask_frames(before prepend/pool) shape={mask_frames.shape}")
            ...
            #check print
            print(f"[predict] mask_frames(after prepend+pool) shape={mask_frames.shape}")
            print(f"[predict] partial_mask(after pool) shape={partial_mask.shape}")
...
#check print
        print(f"[predict] freqs_cos shape={freqs_cos.shape}, freqs_sin shape={freqs_sin.shape}")
        print(f"[predict] freqs_cos_cond shape={freqs_cos_cond.shape}, freqs_sin_cond shape={freqs_sin_cond.shape}")
        print(f"[predict] n_tokens={n_tokens}, target_video_length={target_video_length}")
       ...
       embedded_guidance_scale: {embedded_guidance_scale}
        ...
        #check print
        print(f"[predict] pipeline outputs shape={samples.shape}") # 期望 [B, C(=3), T, H, W]
