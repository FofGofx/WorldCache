#voyager/vae/autoencoder_kl_causal_3d.py
...
class AutoencoderKLCausal3D(ModelMixin, ConfigMixin, FromOriginalVAEMixin):
    r"""
    A VAE model with KL loss for encoding images/videos into latents 
    and decoding latent representations into images/videos.
        ...
        if all(proc.__class__ in ADDED_KV_ATTENTION_PROCESSORS for proc in self.attn_processors.values()):
            ...
            raise ValueError(
                f"Cannot call `set_default_attn_processor` when attention processors are of type \
                    {next(iter(self.attn_processors.values()))}"
            )
    ...
    def spatial_tiled_encode(self, x: torch.FloatTensor, return_dict: bool = True, return_moments: bool = False) \
        -> AutoencoderKLOutput:
    ...
    def spatial_tiled_decode(self, z: torch.FloatTensor, return_dict: bool = True) \
        -> Union[DecoderOutput, torch.FloatTensor]:
        r"""
    ...
    def temporal_tiled_encode(self, x: torch.FloatTensor, return_dict: bool = True) -> AutoencoderKLOutput:
        ...
        for i in range(0, T, overlap_size):
            ...
            if self.use_spatial_tiling \
            and (tile.shape[-1] > self.tile_sample_min_size or tile.shape[-2] > self.tile_sample_min_size):
    ...
    def temporal_tiled_decode(self, z: torch.FloatTensor, return_dict: bool = True) \
        ...
        for i in range(0, T, overlap_size):
            ...
            if self.use_spatial_tiling \
            and (tile.shape[-1] > self.tile_latent_min_size or tile.shape[-2] > self.tile_latent_min_size):
