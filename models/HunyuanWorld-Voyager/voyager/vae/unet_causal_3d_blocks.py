#voyager/vae/unet_causal_3d_blocks.py
...
class ResnetBlockCausal3D(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.pre_norm = True  # Force pre-normalization for consistency
        ...
        # Set output groups to input groups if not specified
        if groups_out is None:
            groups_out = groups

        # Initialize first normalization layer based on time embedding type
        if self.time_embedding_norm == "ada_group":
            # Adaptive group normalization with time embedding conditioning
            self.norm1 = AdaGroupNorm(
                temb_channels, in_channels, groups, eps=eps)
        elif self.time_embedding_norm == "spatial":
            # Spatial normalization with time embedding
            self.norm1 = SpatialNorm(in_channels, temb_channels)
        else:
            # Standard group normalization (default)
        ...
        # Initialize time embedding projection based on normalization type
        if temb_channels is not None:
            if self.time_embedding_norm == "default":
                # Simple linear projection for additive time embedding
                self.time_emb_proj = linear_cls(temb_channels, out_channels)
            elif self.time_embedding_norm == "scale_shift":
                # Project to scale and shift parameters for modulation
                self.time_emb_proj = linear_cls(
                    temb_channels, 2 * out_channels)
            elif self.time_embedding_norm == "ada_group" or self.time_embedding_norm == "spatial":
                # No projection needed for adaptive/spatial normalization
                self.time_emb_proj = None
            else:
                raise ValueError(f"Unknown time_embedding_norm : {self.time_embedding_norm} ")
        else:
            # No time embedding provided
            self.time_emb_proj = None

        # Initialize second normalization layer (same logic as first)
        if self.time_embedding_norm == "ada_group":
        ...
        # Initialize upsampling/downsampling layers if needed
        self.upsample = self.downsample = None
        if self.up:
            # Add upsampling layer for upsampling blocks
            self.upsample = UpsampleCausal3D(in_channels, use_conv=False)
        elif self.down:
            # Add downsampling layer for downsampling blocks
            self.downsample = DownsampleCausal3D(
                in_channels, use_conv=False, name="op")

        # Determine if shortcut convolution is needed
        # Use shortcut conv if input and output channels differ
        self.use_in_shortcut = self.in_channels != conv_3d_out_channels if use_in_shortcut is None else use_in_shortcut

        # Initialize shortcut convolution if needed
        self.conv_shortcut = None
        if self.use_in_shortcut:
            # 1x1x1 convolution to match input and output channels
