#WorldScore/worldscore/benchmark/helpers/evaluator.py
import json
import os
...
def process_batch(
    ...
    # Set seed for reproducibility in batch processing
    seed = config.get('seed', 3047)
    set_seed(seed)
...
class Evaluator(BaseEvaluator):
    ...
    def __init__(self, config):
        ...
        # Set seed for reproducibility (use seed from config if available, otherwise default)
        seed = config.get('seed', 3047)
        set_seed(seed)
        
        # 使用 WORLDCACHE_OUTPUT_SUFFIX 统一输出目录命名逻辑
        ...
        if worldcache_suffix:
            if worldcache_suffix.startswith('_'):
                output_dir = worldcache_suffix[1:]
            else:
                output_dir = output_dir + worldcache_suffix
        
        self.root_path = Path(
            f"{config['runs_root']}/{output_dir}/{config['visual_movement']}"
        )
    ...
    def data_exists(self, root_path):
        ...
        failure_marker = os.path.join(root_path, "generation_failed.txt")
        if os.path.exists(failure_marker):
            print(f"-- Generation failed marker found in {root_path}, skipping evaluation")
            return False
