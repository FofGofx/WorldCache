...
class DataLoader:
    ...
    def load_data(self, image_data, regenerate):
        ...
        if image_data["visual_movement"] == "static":
            ...
            # Check for Worldcache output suffix from environment variable
            worldcache_suffix = os.environ.get('WORLDCACHE_OUTPUT_SUFFIX', '')
            if worldcache_suffix:
                # 如果worldcache_suffix以下划线开头，去掉下划线并替换output_dir
                # 这样可以实现: worldscore_output + _worldcache_p30_c60_n2_e20 → worldcache_p30_c60_n2_e20
                if worldcache_suffix.startswith('_'):
                    output_dir = worldcache_suffix[1:]  # 去掉前导下划线，直接替换
                else:
                    output_dir = output_dir + worldcache_suffix  # 追加模式（向后兼容）
